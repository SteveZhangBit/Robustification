...
fun <I> parse(input: InputStream, transformer: (String) -> I): CompactSupDFA<I> {
  ...
  val alphabets = mutableMapOf<String, Pair<Boolean, Boolean>>() // event -> <controllable, observable>
  var isDFA = true
    ...
    for (i in 0 until numStates) {
      ...
      try {
        ...
        for (j in 0 until stateTuple[2].toInt()) {
          ...
          if (transTuple[0] !in alphabets)
            alphabets[transTuple[0]] = attr
          else if (alphabets[transTuple[0]] != attr)
