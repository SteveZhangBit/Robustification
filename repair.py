...
class Repair:
    def __init__(self, sys, env, safety, preferred, progress, alphabet, controllable, observable, verbose=False, alg="pareto"):
        ...
        self.env = list(map(lambda x: self.fsp2fsm(x, alphabet, alphabet), env))
        self.env = self.env[0] if len(self.env) == 1 else d.composition.parallel(*self.env)
        # compose the plant
        self.plant = d.composition.parallel(self.sys, self.env)
    ...
    ########################################################################
    # Helper conversion functions for LTS object, FSP file, FSM file/object 
    ########################################################################
...
class Solutions:
    ...
    def next_least_to_remove(self):
        ...
        l1 = [d for d in self.repair.preferred[PRIORITY1] if d in self.D_max]
        l2 = [d for d in self.repair.preferred[PRIORITY2] if d in self.D_max]
        l3 = [d for d in self.repair.preferred[PRIORITY3] if d in self.D_max]
    ...
    def compute_util_cost(self, preferred_behavior, min_controllable, min_observable):
        """
        ...
        preferred = sum(map(lambda x: self.weight_dict[x], preferred_behavior))
        cost = sum(map(lambda x: self.weight_dict[x][0], min_controllable)) +\
               sum(map(lambda x: self.weight_dict[x][1], min_observable))
        return preferred, cost
    
    def minimize(self, minS, controllable, observable, preferred):
        if self.repair.alg == "pareto":
            return self.minimize_pareto(minS, controllable, observable, preferred)
        if self.repair.alg == "fast":
    ...
    def minimize_pareto(self, minS, controllable, observable, preferred):
        ...
        high_cost_controllable = [c for c in controllable if c in self.repair.controllable[PRIORITY3]] # High cost controllable events
        medium_cost_controllable = [c for c in controllable if c in self.repair.controllable[PRIORITY2]] # Medium cost controllable events
        low_cost_controllable = [c for c in controllable if c in self.repair.controllable[PRIORITY1]] # Low cost controllable events

        high_cost_observable = [o for o in observable if o in self.repair.observable[PRIORITY3]] # High cost Observable Events
        medium_cost_observable = [o for o in observable if o in self.repair.observable[PRIORITY2]] # Medium cost Observable Events
        low_cost_observable = [o for o in observable if o in self.repair.observable[PRIORITY1]] # Low cost Observable Events
        ...
        for i in range(3):
            ...
            while (len(gp_list) != 0 and j < (len(actions_dict[i][0]) + len(actions_dict[i][1]))):
                j += 1 # iterate the counter for how many events have been removed of the current priority
                ...
                for event_dict in p_list:
                    ...
                    if self.repair.check_preferred(sup_plant_raw, event_dict["c"], event_dict["o"], preferred, fail_stop=True):
                        minS = self.repair.construct_supervisor(sup_plant, event_dict["c"], event_dict["o"])
                        minS = self.repair.lts2fsm(minS, event_dict["c"], event_dict["o"])
    ...
    def minimize_fast(self, minS, controllable, observable, preferred):
        high_cost_controllable = [(c, 'c') for c in controllable if c in self.repair.controllable[PRIORITY3]] # High cost controllable events
        medium_cost_controllable = [(c, 'c') for c in controllable if c in self.repair.controllable[PRIORITY2]] # Medium cost controllable events
        low_cost_controllable = [(c, 'c') for c in controllable if c in self.repair.controllable[PRIORITY1]] # Low cost controllable events

        high_cost_observable = [(o, 'o') for o in observable if o in self.repair.observable[PRIORITY3]] # High cost Observable Events
        medium_cost_observable = [(o, 'o') for o in observable if o in self.repair.observable[PRIORITY2]] # Medium cost Observable Events
        low_cost_observable = [(o, 'o') for o in observable if o in self.repair.observable[PRIORITY1]] # Low cost Observable Events
        ...
        for e, t in search_seq:
            ...
            if sup_plant == None:
                continue
            # add minimization if preferred behavior maintained
            if self.repair.check_preferred(sup_plant_raw, tmp_controllable, tmp_observable, preferred, fail_stop=True):
                cur_sup_plant = sup_plant
                cur_controllable = tmp_controllable
                cur_observable = tmp_observable
        if cur_sup_plant != None:
            minS = self.repair.construct_supervisor(cur_sup_plant, cur_controllable, cur_observable)
            minS = self.repair.lts2fsm(minS, cur_controllable, cur_observable)
        return [(minS, cur_controllable, cur_observable)]

    def __iter__(self):
        start_time = datetime.now()
        print(datetime.now(), "====================================>")
        print(start_time, "Initializing search by using", self.repair.alg, "search...")

        # collect all preferred behavior
        preferred = []
        for key in self.repair.preferred:
            preferred.extend(self.repair.preferred[key])
        ...
        self.weight_dict = self.repair.compute_weights()
        ...
        if sup_plant == None:
            ...
            self.D_max = self.repair.check_preferred(sup_plant_raw, controllable, observable, preferred)
            print(datetime.now(), "Maximum fulfilled preferred behavior:", self.D_max)

            # remove those absolutely unused controllable and observable events
            self.minS, self.controllable_max, self.observable_max = self.repair.remove_unnecessary(sup_plant, controllable, observable)
            print(datetime.now(), "Start search from events:")
            print("\tEc:", self.controllable_max)
            print("\tEo:", self.observable_max)

            # initialize the least cost experienced overall as a very negative cost, this stands for -infinity
            self.min_cost = -1_000_000_000
    ...
    def __next__(self):
        ...
        for remove_behavior in D_rm_sets:
            ...
            D_max_subset = set(self.D_max) - set(remove_behavior)
            # find result that is found by minimizing
            for sup, min_controllable, min_observable in self.minimize(self.minS, self.controllable_max, self.observable_max, D_max_subset):
                # compute the total utility and the cost of such a minimization
                # FIXME: cost only need to compute once
                utility_preferred, cost = self.compute_util_cost(D_max_subset, min_controllable, min_observable)
        ...
        if min_cost_bracket > self.min_cost:
            ...
            for c in possible_controllers:
                c["M_prime"] = self.repair.compose_M_prime(c["M_prime"], c["controllable"], c["observable"])
                if self.repair.alg == "pareto":
            ...
            print(datetime.now(), "No new solution found in this iteration.")
            return []
